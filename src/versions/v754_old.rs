...
protocol_impl! {
        ...
        from_event {
            fn try_from(item: ServerDifficultyUpdate) -> TetsuResult<ServerDifficultyUpdatePacket> {
                Ok(ServerDifficultyUpdatePacket {
                    difficulty: v47_old::internal::difficulty_to_byte(&item.difficulty),
        ...
        to_event {
            fn try_from(item: ServerDifficultyUpdatePacket) -> TetsuResult<Event> {
                Ok(Event::ServerDifficultyUpdate(ServerDifficultyUpdate {
                    difficulty: v47_old::internal::byte_to_difficulty(item.difficulty),
                    difficulty_locked: item.difficulty_locked
                }))
            }
        }
    ...
    (0x1F) ClientBound Play KeepAlivePacket: KeepAlive {
        from_event {
        ...
        from_event {
            fn try_from(item: JoinGame) -> TetsuResult<JoinGamePacket> {
                Ok(JoinGamePacket {
                    ...
                    gamemode: v47_old::internal::gamemode_to_byte(&item.gamemode) | (if item.is_hardcore { 0x80 } else { 0x00 }),
        ...
        to_event {
            fn try_from(item: JoinGamePacket) -> TetsuResult<Event> {
                Ok(Event::JoinGame(JoinGame {
                    id: item.id,
                    is_hardcore: item.is_hardcore,
                    gamemode: v47_old::internal::byte_to_gamemode(item.gamemode),
    ...
    (0x1F) ServerBound Play KeepAliveResponsePacket: KeepAlive {
        from_event {
